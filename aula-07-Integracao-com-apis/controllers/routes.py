...
def init_app(app):
    ...
    def apigames():
        urlAPI = 'https://www.freetogame.com/api/games' #pegando url API e colocando como resultado
        # Enviando uma requisição para a API
        resposta = urllib.request.urlopen(urlAPI) #abrindo a url da API
        # Lendo os dados
        dados = resposta.read()
        # Convertendo daods de JSON para dicionario
        listaJogos = json.loads(dados)
        # Rendezirando a página e enviando a lista de jogos
